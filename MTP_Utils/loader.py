...
def get_loader(input_ids, attention_masks, token_type_ids, labels, batchsize):
    ...
    """
    array = utils.TensorDataset(input_ids, 
                                attention_masks, 
                                token_type_ids,
                                labels)
    
    loader = utils.DataLoader(array, 
                              batch_size=batchsize, 
                              shuffle=True)
    return loader

def get_loader_joint(input_ids, attention_masks, token_type_ids, 
                     input_ids2, attention_masks2, token_type_ids2, 
                     labels, batchsize):
    """ Converts input values into a dataloader
    Arguments:
    input_ids (Tensors): Sentences converted to input ids
